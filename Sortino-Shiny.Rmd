...
```{r}
helpText("Choose 5 stocks and their % allocation")
...
helpText("Choose a start date and MAR")
...
rolling_sortino <-
  rollapply(portfolio_returns_xts, 24, function(x) SortinoRatio(x, MAR = MAR)) %>% 
  `colnames<-`("24-rolling")
})
...
renderHighchart({
  ...
  hc_add_series(rolling_sortino(), name = "Sortino", color = "blue") %>%
